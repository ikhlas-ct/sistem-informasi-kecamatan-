...
<style>
    ...
    .meta-label { min-width:120px; color:#94a3b8; font-weight:600; flex-shrink:0; }
    .meta-value { color:#1e293b; }

    /* Status badges */
    .badge-status { font-size:.75rem; padding:4px 12px; border-radius:20px; font-weight:700; }
    ...
    /* Foto grid */
    .foto-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(110px,1fr)); gap:8px; }
    .foto-thumb { width:100%; height:100px; object-fit:cover; border-radius:10px;
        cursor:pointer; transition:transform .15s,box-shadow .15s;
        border:1.5px solid #e2e8f0; }
    .foto-thumb:hover { transform:scale(1.04); box-shadow:0 4px 12px rgba(0,0,0,.15); }
    ...
    .btn-open { display:inline-flex; align-items:center; gap:5px; background:#e0f2fe;
        color:#0369a1; border:none; border-radius:8px; padding:5px 12px;
        font-size:.75rem; font-weight:600; text-decoration:none; transition:all .15s;
        white-space:nowrap; flex-shrink:0; }
    .btn-open:hover { background:#0369a1; color:#fff; }
    .btn-download { display:inline-flex; align-items:center; gap:5px; background:#dcfce7;
        color:#166534; border:none; border-radius:8px; padding:5px 12px;
        font-size:.75rem; font-weight:600; text-decoration:none; transition:all .15s;
        white-space:nowrap; flex-shrink:0; }
    .btn-download:hover { background:#166534; color:#fff; }

    /* Balasan section */
    .balasan-box { background:linear-gradient(135deg,#f0fdf4,#dcfce7);
        border:1.5px solid #bbf7d0; border-radius:12px; padding:18px 20px; }
    .balasan-header { display:flex; align-items:center; gap:8px; margin-bottom:12px; }
    .balasan-avatar { width:36px; height:36px; border-radius:50%; background:#166534;
        display:flex; align-items:center; justify-content:center;
        color:#fff; font-size:.85rem; flex-shrink:0; }
    .balasan-meta-name { font-size:.83rem; font-weight:700; color:#1e293b; }
    .balasan-meta-date { font-size:.72rem; color:#64748b; }
    .balasan-body { font-size:.85rem; color:#374151; line-height:1.6; white-space:pre-wrap; }

    /* Lampiran balasan */
    .balasan-lamp-item { display:flex; align-items:center; gap:10px; padding:8px 12px;
        border:1.5px solid #bbf7d0; border-radius:9px; margin-top:6px; background:#fff; }
    ...
    #lightbox img { max-width:90vw; max-height:85vh; border-radius:8px; box-shadow:0 8px 32px rgba(0,0,0,.5); }
    ...
    #lb-download { background:rgba(255,255,255,.15); color:#fff; border:1.5px solid rgba(255,255,255,.3);
        border-radius:8px; padding:6px 14px; font-size:.8rem; text-decoration:none; }
    #lb-download:hover { background:rgba(255,255,255,.25); color:#fff; }
</style>
@endsection
    ...
    <div class="ph-card">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <a href="{{ route('pengaduan.index') }}"
               class="btn btn-sm btn-light" style="border-radius:10px;border:1.5px solid #e2e8f0;font-size:.82rem;">
        ...
        <div class="col-lg-8">
            ...
            <div class="card section-card">
                <div class="card-body">
                    ...
                    <div style="font-size:.88rem;color:#1e293b;line-height:1.7;white-space:pre-wrap;">{{ $pengaduan->deskripsi }}</div>
                </div>
            </div>
                            ...
                            <div class="foto-grid mb-4">
                                @foreach($gambarList as $lmp)
                                    ...
                                    <img src="{{ asset('storage/' . $lmp->path) }}"
                                         ...
                                         data-name="{{ basename($lmp->path) }}"
                                         alt="{{ basename($lmp->path) }}"
                                ...
                                <div style="margin-top:14px;border-top:1px solid #bbf7d0;padding-top:12px;">
                                    ...
                                    @if($balGambar->count())
                                        <div class="foto-grid mb-3">
                                            @foreach($balGambar as $blmp)
                                                <img src="{{ asset('storage/' . $blmp->path) }}"
                                                     class="foto-thumb"
                                    ...
                                    @foreach($balFile as $blmp)
                                        <div class="balasan-lamp-item">
                                            ...
                                            <span style="font-size:.8rem;font-weight:600;flex:1;
                                                overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
                                                {{ basename($blmp->path) }}
                                            </span>
                                            <a href="{{ asset('storage/' . $blmp->path) }}"
                                               target="_blank" class="btn-open" style="font-size:.72rem;">
                                                <i class="fas fa-external-link-alt"></i> Buka
                                            </a>
                                            <a href="{{ asset('storage/' . $blmp->path) }}"
                                               download class="btn-download" style="font-size:.72rem;">
                                                <i class="fas fa-download"></i> Unduh
                                            </a>
                                        </div>
                                    @endforeach
            ...
            @else
                ...
                <div class="card section-card">
                    <div class="card-body text-center py-4">
                        <div style="width:50px;height:50px;border-radius:50%;background:#f1f5f9;
                            display:flex;align-items:center;justify-content:center;
                    ...
                    <div class="card-body">
                        ...
                        @if($pengaduan->alamat)
                            <div style="font-size:.78rem;color:#64748b;margin-top:8px;">
                                <i class="fas fa-map-pin me-1 text-danger"></i>{{ $pengaduan->alamat }}
                            </div>
                        @endif
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const map = L.map('map', { zoomControl:true, dragging:true }).setView([lat, lng], 15);

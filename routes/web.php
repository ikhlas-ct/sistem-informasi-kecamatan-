...
Route::middleware('auth')->group(function () {
    ...
    // ── INDEX ──────────────────────────────────────────────────
    Route::get('kelola/konten/{jenis}', [KontenController::class, 'index'])
        ->name('konten.index');

    Route::get('kelola/konten/{jenis}/create', [KontenController::class, 'create'])
        ->name('konten.create');

    Route::post('kelola/konten/{jenis}', [KontenController::class, 'store'])
        ->name('konten.store');

    Route::get('kelola/konten/{jenis}/{slug}', [KontenController::class, 'show'])
        ->name('konten.show');

    Route::get('kelola/konten/{jenis}/{slug}/edit', [KontenController::class, 'edit'])
        ->name('konten.edit');

    Route::put('kelola/konten/{jenis}/{id_konten}', [KontenController::class, 'update'])
        ->name('konten.update');

    Route::delete('kelola/konten/{jenis}/{id_konten}', [KontenController::class, 'destroy'])
        ->name('konten.destroy');

    Route::post('kelola/konten/{jenis}/{id_konten}/approve', [KontenController::class, 'approve'])
        ->name('konten.approve');

    // Summernote tidak perlu prefix
    Route::post('blog/upload-image', [KontenController::class, 'uploadImage'])
        ->name('blog.upload.image');
    Route::post('blog/delete-image', [KontenController::class, 'deleteImage'])
...
Route::prefix('camat')->middleware(['auth', 'role:camat'])->group(function () {
    ...
    Route::get('/website/setting', [KecamatanSettingController::class, 'edit'])
        ->name('kecamatan.setting.edit');

    // ── Simpan semua pengaturan (PUT) ─────────────────────────
    Route::put('/website/setting', [KecamatanSettingController::class, 'update'])
        ->name('kecamatan.setting.update');

    // ── Upload gambar inline Summernote (POST) ────────────────
    Route::post('/website/setting/upload-image', [KecamatanSettingController::class, 'uploadImage'])
        ->name('kecamatan.setting.upload-image');

    // ── Hapus gambar inline Summernote (POST) ─────────────────
    Route::post('/website/setting/delete-image', [KecamatanSettingController::class, 'deleteImage'])
        ->name('kecamatan.setting.delete-image');
...
Route::middleware(['auth', 'role:pegawai,camat'])->group(function () {
    ...
    // Laporan Surat Keterangan Miskin
    ...
    // Update - simpan perubahan pegawai
